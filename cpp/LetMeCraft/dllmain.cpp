...
#include "lmc/InputDevice.hpp"
#include "lmc/StationIcons.hpp"
...
public:
    ...
    {
        // Single concise load line. Detailed diagnostics are gated behind kDebugLog (Common.hpp).
        Output::send<LogLevel::Verbose>(STR("[LetMeCraft] v1.2.0 loaded (eviction v1.1.0).\n"));

        register_keydown_event(Input::Key::E, [this]() {
            m_input.note_keyboard();
    ...
    {
        m_input.update();
        if (m_input.consume_y_press()) { queue_manual_request(STR("gamepad Y")); }
    }
...
private:
    ...
    auto update_glyph() -> void
    {
        ...
        m_glyph.update(glyph_active, glyph_avatar, m_input.is_gamepad(), m_glyph_name, m_glyph_icon);
    }
